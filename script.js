...
operatorCards.forEach(card => {
  card.addEventListener('click', () => {
    ...
    document.getElementById('instruction-list-orange')?.classList.toggle('hidden', operator !== 'orange');
    document.getElementById('instruction-list-mvola')?.classList.toggle('hidden', operator !== 'mvola');
...
function aiSuggest(topic) {
  const responses = {
    tarifs: 'Nos tarifs commencent à 25 000 Ar/mois pour le plan Basic. Le plan Pro à 45 000 Ar/mois inclut le module BTP et la gestion de stock. Nous proposons aussi une licence à vie à 820 000 Ar.',
    diaspora: 'Vous pouvez payer depuis l\'étranger via TapTapSend (sans frais) ou Sendwave. Envoyez l\'argent au +261 32 02 198 89 et uploadez votre preuve de paiement.',
    essai: 'Oui ! Vous bénéficiez de 30 jours d\'essai gratuit avec toutes les fonctionnalités. Aucune carte bancaire requise.'
  };
...
// Apply to cards
document.querySelectorAll('.trust-card, .pricing-card, .feature-card, .contact-card, .service-card').forEach(el => {
  el.style.opacity = '0';
